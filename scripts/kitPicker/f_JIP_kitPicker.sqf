...
KitPickInit = {
    ...
    _idx = COMBO lbAdd "Platoon Sergeant"; COMBO lbSetData [_idx, "secco"];
    ...
    _idx = COMBO lbAdd "Rotary Wing Aircrew"; COMBO lbSetData [_idx, "rotarycrew"];
    ...
    _idx = COMBO lbAdd "MAT Gunner"; COMBO lbSetData [_idx, "matgun"];
    _idx = COMBO lbAdd "MAT Assistant"; COMBO lbSetData [_idx, "matammo"];
	...
	_idx = COMBO lbAdd "SHAT Gunner"; COMBO lbSetData [_idx, "shatgun"];
	_idx = COMBO lbAdd "SHAT Assistant"; COMBO lbSetData [_idx, "shatass"];

	// Special Roles
	COMBO lbAdd "--Specialist Roles--";
	_idx = COMBO lbAdd "Scoped Rifleman"; COMBO lbSetData [_idx, "rmsc"];
	_idx = COMBO lbAdd "Heavy Grenadier"; COMBO lbSetData [_idx, "heavygren"];
    _idx = COMBO lbAdd "SpecOps Team Leader"; COMBO lbSetData [_idx, "spectl"];
    _idx = COMBO lbAdd "SpecOps Rifleman"; COMBO lbSetData [_idx, "specrm"];
	...
	_idx = COMBO lbAdd "AM Sniper"; COMBO lbSetData [_idx, "amsniper"];
    _idx = COMBO lbAdd "Spotter"; COMBO lbSetData [_idx, "spotter"];
    _idx = COMBO lbAdd "Engineer"; COMBO lbSetData [_idx, "engi"];
    _idx = COMBO lbAdd "Demo Specialist"; COMBO lbSetData [_idx, "demoman"];
...
KitPicker_DistanceChecker = {
	private ["_startPos","_action"];
	waitUntil {!isNull player && player == player};
	if (r_isJIP) then {
		_startPos = getPos player;
		waitUntil {sleep 1; ((getPos player) distance _startPos) > 50;};
		if (!isNil "F3_JIP_reinforcementOptionsAction") then { player removeAction F3_JIP_reinforcementOptionsAction; };
	};
};